...
local folder = "dir/"
local vocabulary_fn = folder..'vocabulary_en'
local inv_vocabulary_fn = folder..'inv_vocabulary_en'
local filtered_sentences_fn = folder..'enc_in_seq_en'
local filtered_sentences_indexes_fn = folder..'enc_in_seq_indexes_en'
local src_filename = folder..'source.txt'-- 原始文章
local should_reverse = false

local fd = io.lines(src_filename)
local words_count = {}
local words = {}
local vocab_size = 8818
...
while line do
  ...
  local sentence = {}
  for _, word in pairs(split_en(line, " ")) do
